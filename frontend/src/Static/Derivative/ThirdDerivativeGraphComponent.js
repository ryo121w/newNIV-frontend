...
const ThirdDerivativeGraphComponent = ({ setIsLoading }) => {
    ...
    const onSubmit = async (e) => {
        ...
        try {
            const response = await axios.post(`${BACKEND_URL}api/third_derivative_graph/`);
    ...
    const onDownload = async (event) => {
        ...
        try {
            const response = await fetch(`${BACKEND_URL}api/third_derivative_download/`);
    ...
    return (
        <div className={styles['Graph-container']}>
            <div className={styles['Graph-left-side']}>
                <h5 className={styles['title']}>三次微分</h5>
